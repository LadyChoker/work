...
static struct sockaddr* init_sockaddr_in_any(JNIEnv *env, struct sockaddr_in *sin) {
...
(JNIEnv *env, jclass cls, jint socket, jbyteArray multiaddr,jbyteArray interface )
{
	...
	/*Reservo memoria para la estructura del grupo*/
	memset((void *) &group, 0, sizeof(struct ip_mreq));
	/*Copio la direccion multicast*/
	buf1 = (*env)->GetByteArrayElements(env, multiaddr, NULL);
	memcpy(&(group.imr_multiaddr.s_addr), buf1, sizeof(group.imr_multiaddr.s_addr));
	/*Copio la direccion del interfaz*/
	buf2 = (*env)->GetByteArrayElements(env, interface, NULL);
	memcpy(&(group.imr_interface.s_addr), buf2, sizeof(group.imr_interface.s_addr));
	  int reuse = 1;
...
/* specified as INADDR_ANY. */
     socklen = sizeof(sin);
    saddr = init_sockaddr_in_any(env, &sin);
 
   bind(socket, saddr, socklen);
printf("Binding datagram socket...OK.\n");

  int ret;
  ret= setsockopt(socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&group, sizeof(group));
  if(ret < 0)
    {
  ...
  (*env)->ReleaseByteArrayElements(env, multiaddr, buf1, JNI_ABORT);
	 
  (*env)->ReleaseByteArrayElements(env, interface, buf2, JNI_ABORT);
  /*int ret=1;*/
	return ret;	
}
